...
face_comparision_threshold = 0.77
...
def facecompare(selfie,proof):
    ...
    selfie_rep = {}
    proof_rep = {}
    selfieTask = threading.Thread(target=getRep, args=(selfie,1,selfie_rep,))
    selfieTask.start()
    proofTask = threading.Thread(target=getRep, args=(proof,2,proof_rep,))
    proofTask.start()
...
def getFaceRep(selfie):
    response = {}
    selfie_rep = getRep(selfie,1,response)
    status = selfie_rep['status']
    return selfie_rep
...
def getRep(img, fromwhere,response):
    # response = {}
